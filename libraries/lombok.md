...
### lombok.val についてどう思いますか？

使いたければ、Java 11 にアップグレードして var 使いましょう。

<S>
lombok.val は過去には IDE サポートが貧弱だったりして厳しい状況だったこともありました。
現在では IDE サポートも改善されているようです。

ただ、積極的に利用する価値があるのかというと微妙だな、と現時点では思っています。

例えば、以下のようなコードはコンパイルエラーになってしまいます。
左辺で明示できないために、`<>` のようなジェネリクスの省略記法を利用した場合、Object を指定したことになるからです。

    val goa = new ArrayList<>();
    String x = goa.get(0);

IDEA で "Introduce local variable" したほうが val を利用したよりも書く速度が速いんじゃないか、という気がするのもあり、個人的には利用するモチベーションが薄いです。
(vim でちょっとした java コードを書いている時とかは欲しい気がします)

[JEP 286](http://openjdk.java.net/jeps/286) のようなコアに入れようという動きもあるので、そちらが入るのを待ってもいいかも。
</S>
